...
def _ext_hf_space(prompt: str, W: int, H: int, seed: int) -> dict:
    """HuggingFace Space propio — FLUX.1 real, endpoint Gradio /run/generate_api."""
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            logger.warning(f"[HFSpace] HTTP {r.status_code}: {r.text[:200]}")
    except Exception as e:
